...
function LightGraphs.has_edge(bgraph::BipartiteGraph, from::Int64, to::Int64)
    return LightGraphs.has_edge(bgraph.graph, from, to)
end
function LightGraphs.has_vertex(bgraph::BipartiteGraph, v::Integer)
    return LightGraphs.has_vertex(bgraph.graph, v)
end
...
function _identify_separators(
    bgraph::BipartiteGraph, partitions::Vector; cut_selector=LightGraphs.degree
)
    ...
    indices = [indices[i].I[2] for i in 1:length(indices)]   #index of cross edges
