...
struct AggregateMap
    ...
    variable_map::Dict{NodeVariableRef,NodeVariableRef}

    #map constraints in original optigraph to optinode
    constraint_map::Dict{JuMP.ConstraintRef,JuMP.ConstraintRef}  
end

function Base.getindex(agg_map::AggregateMap, vref::NodeVariableRef)
...
function Base.setindex!(
    agg_map::AggregateMap, graph_vref::NodeVariableRef, node_vref::NodeVariableRef
)
...
function aggregate(graph::OptiGraph; return_mapping=true)
    # aggregate into a graph containing a single node.
    node_graph = OptiGraph()
    new_node = add_node(node_graph)
    agg_map = AggregateMap()

    for node in all_nodes(graph)
        _add_to_aggregate_node(new_node, node, agg_map)
    end

    for edge in all_edges(graph)
        _add_to_aggregate_node(new_node, edge, agg_map)
    end

    if return_mapping
        return node_graph, agg_map
    else
        return node_graph
    end
end
