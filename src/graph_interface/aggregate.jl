...
"""
struct AggregateMap
    variable_map::Dict{NodeVariableRef,JuMP.VariableRef}         #map variables in original optigraph to optinode
    constraint_map::Dict{JuMP.ConstraintRef,JuMP.ConstraintRef}  #map constraints in original optigraph to optinode
end

function Base.getindex(agg_map::AggregateMap, vref::JuMP.VariableRef)  #agg_map[node_var] --> combinedd_copy_var
    return agg_map.varmap[vref]
end

function Base.getindex(agg_map::AggregateMap, cref::JuMP.ConstraintRef)
    return agg_map.conmap[cref]
end

#NOTE: Quick fix for aggregating object dictionaries
function Base.getindex(agg_map::AggregateMap, value::Any)
    return value
end

Base.broadcastable(agg_map::AggregateMap) = Ref(agg_map)

function Base.setindex!(
    agg_map::AggregateMap,
    graph_cref::JuMP.ConstraintRef,
    node_cref::JuMP.ConstraintRef,
)
    return agg_map.conmap[node_cref] = graph_cref
end

function Base.setindex!(
    agg_map::AggregateMap, graph_vref::JuMP.VariableRef, node_vref::JuMP.VariableRef
)
    return agg_map.varmap[node_vref] = graph_vref
end
...
function aggregate(graph::OptiGraph)
    ...
    # agg_map = AggregateMap(aggregate_node)
    agg_map = AggregateMap()
    ...
    for node in all_nodes(graph)
        ...
        # node_agg_map = _add_to_aggregate_node!(aggregate_node,optinode,agg_map)  #updates combined_model and agg_map
        _add_to_aggregate_node!(
            aggregate_node, node, agg_map
        )
    end
    ...
    #         JuMP.constraint_object(linkconstraint), agg_map
    #     )
    #     cref = JuMP.add_constraint(aggregate_node, new_constraint)
    #     agg_map.linkconstraintmap[JuMP.constraint_object(linkconstraint)] = cref
    # end

    return aggregate_node, agg_map
end

function aggregate(graph::OptiGraph)
    return to_jump_model(graph)
end
...
function _add_to_aggregate_node!(
    ...
    for var in JuMP.all_variables(add_node)
        ...
        agg_map[var] = new_x               #map variable reference to new reference
    ...
    for (func, set) in constraint_types
        ...
        for constraint_ref in constraint_refs
            ...
            new_constraint = _copy_constraint(constraint, agg_map)
            new_ref = JuMP.add_constraint(aggregate_node, new_constraint)
            agg_map[constraint_ref] = new_ref
        end
    end
    ...
    if nlp != nothing
        ...
        for i in 1:length(nlp.constraints)
            ...
            _splice_nonlinear_variables!(expr, add_node, agg_map)        #splice the variables from var_map into the expression
            ...
            agg_map[constraint_ref] = new_nl_constraint
        end
    end
...
function aggregate(graph::OptiGraph, max_depth::Int64)  #0 means no subgraphs
    ...
    agg_map = AggregateMap()  #old optigraph => new optigraph
    ...
    for graph in reverse(final_parents)  #reverse to start from the bottom
        ...
        for edge in edges
            for linkconstraint in linkconstraints(edge)
                new_con = _copy_constraint(linkconstraint, agg_map)
                JuMP.add_constraint(new_graph, new_con)
            end
        end
    end
    return root_optigraph, agg_map
end
