...
function _copy_objective(
    func::Union{JuMP.GenericAffExpr,JuMP.GenericQuadExpr}, ref_map::AggregateMap
)
...
function _copy_nl_objective(
    node::OptiNode, evaluator::MOI.Nonlinear.Evaluator, reference_map::AggregateMap
)
...
function _splice_nonlinear_variables!(
    expr::Expr, node::OptiNode, reference_map::AggregateMap
)
...
function _copy_constraint_func(func::JuMP.GenericQuadExpr, ref_map::AggregateMap)
    ...
    new_terms = OrderedDict([
        (JuMP.UnorderedPair(ref_map[pair.a], ref_map[pair.b]), coeff) for
        (pair, coeff) in func.terms
    ])
