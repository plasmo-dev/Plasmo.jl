...
function printiterationsummary(s::Solution; singleline=false)
    if singleline
        ...
        print("$(round(s.iterbound[end],4)) \t")
        print("| $(round(s.objval,4)) \t")
        print("$(round(s.bestbound,4)) \t")
        print("| $(round(s.gap*100,2)) %\t")
        print("| $(round(s.clocktime[end],2)) s\n")
    else
        println("-----------------")
        ...
        println("Best Bound : $(s.bestbound)")
        println("Gap : $(round(s.gap*100,2)) %")
        println("Iteration Time : $(round(s.itertime[end],2)) s")
        println("Elapsed Time : $(round(s.clocktime[end],2)) s")
    end
end
