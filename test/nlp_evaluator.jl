...
function _make_plasmo_graph()
    ...
    @NLobjective(n1, Min, sum(n1[:x][i] for i in 1:5)^3)
    ...
    @objective(n2, Min, sum(n2[:x][i]^2 for i in 1:5))
...
function _make_jump_model()
    ...
    @NLobjective(model, Min, sum(x1[i] for i in 1:5)^3 + sum(x2[i]^2 for i in 1:5))
