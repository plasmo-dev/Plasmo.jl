...
function test_hypergraph()
    ...
    @test Plasmo.incident_edges(hyper,1) == [Plasmo.HyperEdge(1,2,3),Plasmo.HyperEdge(1,2),Plasmo.HyperEdge(1,3,4)]

    @test Plasmo.induced_edges(hyper,[1,2,3]) == [Plasmo.HyperEdge(1,2,3),Plasmo.HyperEdge(1,2)]

    @test Plasmo.incident_edges(hyper,[1,2]) == [Plasmo.HyperEdge(1,2,3),Plasmo.HyperEdge(1,3,4)]

    partition_vector = [[1,2,3],[4,5,6]]
    p,cross = Plasmo.identify_edges(hyper,partition_vector)
    @test p == [[Plasmo.HyperEdge(1,2,3), Plasmo.HyperEdge(1,2)],
                [Plasmo.HyperEdge(4,5,6)]]
...
function test_bipartite_graph()
    ...
    p,cross = Plasmo._identify_separators(graph,part_vector;cut_selector=LightGraphs.degree)
    @test p == [[1,4],[3,5]]
    @test cross == [2]
